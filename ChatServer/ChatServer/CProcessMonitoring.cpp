...
CProcessMonitoring::CProcessMonitoring(HANDLE hProcess)
{
	...
	PdhAddCounter(_hPdhQuery, L"\\Process(ChatServer)\\Private Bytes", NULL, &_PdhCounter[en_PROCESS_USER_MEMORY]);
	PdhAddCounter(_hPdhQuery, L"\\Process(ChatServer)\\Pool Nonpaged Bytes", NULL, &_PdhCounter[en_PROCESS_NONPAGED]);
